...
class MyWidget(QWidget):
    ...
    def reqdata(self):
        ...
        result1 = [x for x in values if x%2==0]
        result2 = [x for x in values if x%2==1]
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(sql)
                sql = f"""INSERT INTO {tablename_text} (List, List_even, List_not_even) VALUES ('{str(values)}', '{str(result1)}', '{str(result2)}');"""
                print(sql)
                cursor.execute(sql)
                connection.commit()
